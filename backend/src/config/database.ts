...
export const query = async (sql: string, params: any[] = []): Promise<any[]> => {
  ...
  // Basic translation for SQLite specific queries
  ...
  try {
    ...
    if (err.code === '23505') return []; // Ignore duplicate key errors for seeding
    console.error('Database query error:', err.message, 'SQL:', pgSql);
    return [];
  }
};
...
export const run = async (sql: string, params: any[] = []): Promise<{ lastID: number; changes: number }> => {
  ...
  try {
    ...
    if (err.code === '23505') return { lastID: 0, changes: 0 };
...
export const initDatabase = async () => {
  try {
    ...
    const careersInfo = await query("SELECT column_name FROM information_schema.columns WHERE table_name='careers'");
    const careersCols = careersInfo.map(c => c.column_name);
    if (!careersCols.includes('expire_date')) await run("ALTER TABLE careers ADD COLUMN expire_date TIMESTAMP");
    ...
    const jobAppsInfo = await query("SELECT column_name FROM information_schema.columns WHERE table_name='job_applications'");
    const jobAppsCols = jobAppsInfo.map(c => c.column_name);
    ...
    const admin = await getOne(`SELECT * FROM users WHERE email = $1`, ['admin@nilepixel.com']);
    if (!admin) {
      const passwordHash = await bcrypt.hash('Admin@123', 10);
      
      await run(`INSERT INTO roles (uuid, name, description) VALUES ('role-admin', 'Administrator', 'Full access')`);
      await run(`INSERT INTO roles (uuid, name, description) VALUES ('role-editor', 'Editor', 'Content management')`);
      await run(`INSERT INTO roles (uuid, name, description) VALUES ('role-viewer', 'Viewer', 'Read only')`);

      await run(`
        INSERT INTO users (uuid, first_name, last_name, email, password_hash, role_id, is_active, email_verified)
        VALUES ('usr-admin-001', 'Admin', 'NilePixel', 'admin@nilepixel.com', $1, 1, 1, 1)
      `, [passwordHash]);
      ...
      for (const [key, val, group] of defaultSettings) {
        await run(`INSERT INTO site_settings (setting_key, setting_value, setting_group) VALUES ($1, $2, $3)`, [key, val, group]);
      }

      await run(`
        INSERT INTO company_information (about_summary, mission, vision, history, phone, email, address, working_hours, maps_url)
        VALUES ('NilePixel Technologies is a full-service software development agency operating at the intersection of enterprise engineering, AI, and cloud technology.',
      ...
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('service', 'Web Engineering', 'web-engineering', 'Full stack SaaS & enterprise web apps')`);
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('service', 'Mobile Engineering', 'mobile-engineering', 'Cross-platform iOS and Android apps')`);
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('service', 'Cloud & DevOps', 'cloud-devops', 'Kubernetes, AWS, microservices, and CI/CD')`);
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('service', 'AI Solutions', 'ai-solutions', 'LLMs, machine learning, and automation')`);
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('project', 'FinTech', 'fintech', 'Financial software and payment gateways')`);
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('project', 'Enterprise Systems', 'enterprise-systems', 'ERP, CRM, and operational automation')`);
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('project', 'HealthTech', 'healthtech', 'Healthcare management & telehealth platform')`);
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('blog', 'Engineering', 'engineering', 'Software architecture & clean code')`);
      await run(`INSERT INTO categories (type, name, slug, description) VALUES ('blog', 'AI & Data', 'ai-data', 'Generative AI and data pipelines')`);
      ...
      for (const [tName, tSlug, tCat, tDesc] of techList) {
        await run(`INSERT INTO technologies (name, slug, category, description) VALUES ($1, $2, $3, $4)`, [tName, tSlug, tCat, tDesc]);
      }
      ...
      console.log('NilePixel PostgreSQL schema & initial seed data initialized successfully!');
    }
  } catch (err) {
