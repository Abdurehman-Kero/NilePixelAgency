          ...
          <div className="bg-[#0A1220] border border-[#1B2B44] rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6 space-y-6 shadow-2xl relative">
            ...
            {success ? (
              <div className="bg-[#00E599]/10 border border-[#00E599]/20 p-6 rounded-xl text-center space-y-3">
                ...
                <button onClick={() => setSelectedJob(null)} className="mt-4 px-6 py-2 bg-[#1B2B44] hover:bg-[#23344F] text-white rounded-lg font-semibold transition-colors">
                  Close
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-4 text-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required type="text" value={formData.applicant_name} onChange={e=>setFormData({...formData, applicant_name: e.target.value})} className="w-full bg-[#101C2F] border border-[#23344F] rounded-xl px-4 py-2.5 text-white" />
                  </div>
                  <div>
                    <label className="block text-[#A9B4C5] mb-1.5 font-medium">Email Address *</label>
                    <input required type="email" value={formData.email} onChange={e=>setFormData({...formData, email: e.target.value})} className="w-full bg-[#101C2F] border border-[#23344F] rounded-xl px-4 py-2.5 text-white" />
                  </div>
                  <div>
                    <label className="block text-[#A9B4C5] mb-1.5 font-medium">Phone Number</label>
                    <input type="tel" value={formData.phone} onChange={e=>setFormData({...formData, phone: e.target.value})} className="w-full bg-[#101C2F] border border-[#23344F] rounded-xl px-4 py-2.5 text-white" />
                  </div>
                  <div>
                    <label className="block text-[#A9B4C5] mb-1.5 font-medium">Telegram Username</label>
                    <input type="text" placeholder="@username" value={formData.telegram_username} onChange={e=>setFormData({...formData, telegram_username: e.target.value})} className="w-full bg-[#101C2F] border border-[#23344F] rounded-xl px-4 py-2.5 text-white" />
                  </div>
                  <div>
                    <label className="block text-[#A9B4C5] mb-1.5 font-medium">Resume/CV URL *</label>
                    <input required type="url" placeholder="Link to Google Drive, Dropbox, etc." value={formData.resume_url} onChange={e=>setFormData({...formData, resume_url: e.target.value})} className="w-full bg-[#101C2F] border border-[#23344F] rounded-xl px-4 py-2.5 text-white" />
                  </div>
                  <div>
                    <label className="block text-[#A9B4C5] mb-1.5 font-medium">Portfolio URL</label>
                    <input type="url" value={formData.portfolio_url} onChange={e=>setFormData({...formData, portfolio_url: e.target.value})} className="w-full bg-[#101C2F] border border-[#23344F] rounded-xl px-4 py-2.5 text-white" />
                  </div>
                  <div>
                    <label className="block text-[#A9B4C5] mb-1.5 font-medium">LinkedIn Profile</label>
                    <input type="url" value={formData.linkedin_url} onChange={e=>setFormData({...formData, linkedin_url: e.target.value})} className="w-full bg-[#101C2F] border border-[#23344F] rounded-xl px-4 py-2.5 text-white" />
                  </div>
                </div>
                ...
                <div className="pt-4 flex justify-end gap-3">
                  <button type="button" onClick={() => setSelectedJob(null)} className="px-6 py-3 rounded-xl font-bold text-white hover:bg-[#1B2B44] transition-colors">
                    Cancel
                  </button>
                  <button type="submit" disabled={submitting} className="px-6 py-3 rounded-xl font-bold bg-[#00E599] text-[#0A1220] hover:bg-[#00E599]/90 transition-colors disabled:opacity-50">
